    ...
    <section class="section">
        <div class="container">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 50px; align-items: center; margin-bottom: 60px;">
                <div>
                    <h2 style="font-size: 32px; margin-bottom: 20px;">Welcome to <span style="color: var(--primary-blue);">{{ $settings['about_intro_title'] ?? 'Nayon Engineering' }}</span></h2>
                    <p style="color: var(--light-gray); line-height: 1.8; margin-bottom: 20px;">
                        {{ $settings['about_intro_text_1'] ?? 'Nayon Engineering is your trusted partner for industrial spares, electrical supplies, project management, and installations across Africa and globally.' }}
                    </p>
                    <p style="color: var(--light-gray); line-height: 1.8; margin-bottom: 20px;">
                        {{ $settings['about_intro_text_2'] ?? 'Our comprehensive range of products includes everything from basic electrical components to advanced automation systems, ensuring that we can meet all your industrial and electrical needs under one roof.' }}
                    </p>
                    <p style="color: var(--light-gray); line-height: 1.8;">
                        {{ $settings['about_intro_text_3'] ?? 'We pride ourselves on offering competitive prices, quality products from trusted brands, and exceptional customer service.' }}
                    </p>
                </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px; margin-bottom: 60px;">
                <div style="background-color: var(--card-bg); border-radius: 8px; padding: 40px 30px; text-align: center;">
                    ...
                    <h3 style="margin-bottom: 15px;">{{ $settings['about_mission_title'] ?? 'Our Mission' }}</h3>
                    <p style="color: var(--light-gray); line-height: 1.7;">
                        {{ $settings['about_mission_text'] ?? 'To provide quality electrical products and services that empower industries across Africa and beyond.' }}
                    </p>
                </div>
                <div style="background-color: var(--card-bg); border-radius: 8px; padding: 40px 30px; text-align: center;">
                    <div style="width: 80px; height: 80px; background-color: var(--primary-blue); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                    ...
                    <h3 style="margin-bottom: 15px;">{{ $settings['about_vision_title'] ?? 'Our Vision' }}</h3>
                    <p style="color: var(--light-gray); line-height: 1.7;">
                        {{ $settings['about_vision_text'] ?? 'To be the leading supplier of industrial electrical products and services in Africa.' }}
                    </p>
                </div>
                <div style="background-color: var(--card-bg); border-radius: 8px; padding: 40px 30px; text-align: center;">
                    <div style="width: 80px; height: 80px; background-color: var(--primary-blue); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                    ...
                    <h3 style="margin-bottom: 15px;">{{ $settings['about_values_title'] ?? 'Our Values' }}</h3>
                    <p style="color: var(--light-gray); line-height: 1.7;">
                        {{ $settings['about_values_text'] ?? 'Integrity, quality, innovation, and customer focus drive everything we do.' }}
                    </p>
                </div>
            </div>
            ...
            <div style="margin-bottom: 60px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                    @php
                        ...
                        if (!empty($settings['about_industries'])) {
                            $industries = [];
                            $lines = explode("\n", $settings['about_industries']);
                            foreach ($lines as $line) {
                                $line = trim($line);
                                if (empty($line)) continue;
                                $parts = explode('|', $line);
                                $name = trim($parts[0]);
                                $icon = isset($parts[1]) ? trim($parts[1]) : 'fa-cog';
                                $industries[$name] = $icon;
                            }
                        }
                    @endphp
                    @foreach($industries as $industry => $icon)
